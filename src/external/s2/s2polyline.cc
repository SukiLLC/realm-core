...
struct SearchState {
  ...
  // chosen has no special meaning.  It lives on the type rather than in a
  // std::less<SearchState> specialization because libc++ detects the primary
  // std::less template and lowers it to a direct operator< call, which would
  // silently bypass any specialization.
